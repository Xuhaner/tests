...
public class ObjectBeanService {
	...
	public static int intsertObjectBean(ObjectBean bean) throws Exception {
		...
		try {
			...
			sql = "insert into wj_object(title,discribe,state,remark,shareFlag,team,author) values(?,?,?,?,?,?,?)";
			...
			stmt.setString(5, bean.getShareFlag());
	...
	public static int updateObjectBean(ObjectBean bean) {
		...
		String sql1 = "update wj_object set title = '" + bean.getTitle()
				+ "',discribe = '" + bean.getDiscribe() + "',shareFlag = '" + bean.getShareFlag() + "'" + ",team ='" + bean.getTeam() + "'" + 
	...
	public static List ListTeamObjectBean(String teamname) {
		...
		List objList=new LinkedList();
		try {
			String sql = "select oid,title,createtime,state,team,author from wj_object where team = '" + teamname + "' and state = 1 order by oid desc";
			con=dbcon.getConnection();
			stm=con.createStatement();
			rs = stm.executeQuery(sql);
			while(rs.next()){
...
//	 查看问卷;
	public static List ListOtherObjectBean() {
		DBConnection dbcon = new DBConnection();
		Connection con = null;
		Statement stm = null;
		ResultSet rs = null;
		List objList=new LinkedList();
		try {
			String sql = "select oid,title,createtime,state,team,author from wj_object where team is null or team = '" + "' and state = 1  order by oid desc";
			con=dbcon.getConnection();
			stm=con.createStatement();
			rs = stm.executeQuery(sql);
			while(rs.next()){
...
//	 查看问卷;
	public static List ListMyObjectBean(String username) {
		DBConnection dbcon = new DBConnection();
		Connection con = null;
		Statement stm = null;
		ResultSet rs = null;
		List objList=new LinkedList();
		try {
			String sql = "select oid,title,createtime,state,team,author from wj_object where author = '" + username + "' order by oid desc";
			con=dbcon.getConnection();
			stm=con.createStatement();
			rs = stm.executeQuery(sql);
			while(rs.next()){
	...
	public static ObjectBean findObjectBeanByID(int ID) {
		...
		try {
			String sql = "select oid,title,discribe,state,shareFlag,team,author from wj_object where oid='" + ID
			+ "'";
			con=dbcon.getConnection();
			stm=con.createStatement();
			 rs = stm.executeQuery(sql);
			 ...
			 String shareFlag = rs.getString("shareFlag");
			 int state=rs.getInt("state");
			 String team = rs.getString("team");
			 String author = rs.getString("author");
